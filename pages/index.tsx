...
const Home: NextPage = () => {
  ...
  // below useState gives us the NFTs(i.e FarmerTools) that is staked in the staking contract 
  const { data: equippedTools } = useContractRead(
    stakingContract, 
    "getStakeInfo",
    [address] // address of connected wallet
  );

  const { data: rewardBalance } = useContractRead(rewardContract, "balanceOf", [address]); // address of connected wallet
